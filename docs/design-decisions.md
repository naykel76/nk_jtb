...
## Responsive Prefix Scope (review)
...
## `on-` and `to-` Are Not in the Main Build Controller (review)
...
## Physical Position Utilities (review)
...
## Color Scale in Utility Output (review)
...
## Layer 4 (Responsive + State) Is Always Generated (review)
...
## `$split-properties` Must Be Maintained Manually (review)
...
## No `to-xxl` in `to-breakpoint()` Mixin (review)
...
## `$text-color-map` in `_typography.scss` (review)
...
## Semantic z-index Tokens (review)
...
## No Generated Class Reference (review)
